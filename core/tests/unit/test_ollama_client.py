...
async def test_health_failure(client):
    respx.get("http://127.0.0.1:11435/").mock(
...
async def test_stream_chat_yields_tokens(client):
    stream_content = (
        ...
        + "\n"
        + json.dumps(
            {"model": "gemma:2b", "created_at": "2024-01-01T00:00:02Z", "done": True}
        )
        + "\n"
    )
    respx.post("http://127.0.0.1:11435/api/chat").mock(
        return_value=Response(200, content=stream_content)
    )

    tokens = []
    async for token in client.stream_chat(
        model="gemma:2b", messages=[{"role": "user", "content": "hi"}]
    ):
        tokens.append(token)
...
async def test_pull_model_progress(client):
    ...
    respx.post("http://127.0.0.1:11435/api/pull").mock(
        return_value=Response(200, content=stream_content)
    )
...
async def test_list_models_success(client):
    ...
    respx.get("http://127.0.0.1:11435/api/tags").mock(

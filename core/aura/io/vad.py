...
class SileroVAD:
    ...
    def _process_inference(
        self, session: ort.InferenceSession, audio_float32: np.ndarray
    ) -> bool:
        """
        ...
        inputs = {"input": audio_float32, "sr": sr, "h": h, "c": c}
    ...
    async def is_speech(self, audio_chunk: bytes) -> bool:
        ...
        try:
            ...
            is_voice = await asyncio.to_thread(
                self._process_inference, session, audio_float32
            )

...
class OpenApplicationTool(Tool):
    ...
    parameters = {
        ...
        "properties": {
            "name": {
                "type": "string",
                "description": "The name or path of the application to open",
            }
        },
        "required": ["name"],
    }
    ...
    async def execute(self, name: str) -> ToolResult:
        try:
            ...
            if success:
                return ToolResult(
                    success=True, output=f"Application '{name}' launched successfully."
                )
            return ToolResult(
                success=False,
                output=None,
                error=f"Failed to launch application '{name}'.",
            )
...
class FocusWindowTool(Tool):
    ...
    async def execute(self, title: str) -> ToolResult:
        try:
            ...
            if success:
                return ToolResult(
                    success=True, output=f"Window '{title}' focused successfully."
                )
            return ToolResult(
                success=False,
                output=None,
                error=f"Window '{title}' not found or could not be focused.",
            )
        except Exception as e:
